...
public class EdgeGrpcService extends EdgeRpcServiceGrpc.EdgeRpcServiceImplBase implements EdgeRpcService {
    ...
    private void onEdgeConnect(EdgeId edgeId, EdgeGrpcSession edgeGrpcSession) {
        ...
        save(tenantId, edgeId, DefaultDeviceStateConstants.ACTIVITY_STATE, true);
        long lastConnectTs = System.currentTimeMillis();
        save(tenantId, edgeId, DefaultDeviceStateConstants.LAST_CONNECT_TIME, lastConnectTs);
    ...
    private void onEdgeDisconnect(Edge edge, UUID sessionId) {
        ...
        if (toRemove.getSessionId().equals(sessionId)) {
            ...
            save(tenantId, edgeId, DefaultDeviceStateConstants.ACTIVITY_STATE, false);
            long lastDisconnectTs = System.currentTimeMillis();
            save(tenantId, edgeId, DefaultDeviceStateConstants.LAST_DISCONNECT_TIME, lastDisconnectTs);
    ...
    private void pushRuleEngineMessage(TenantId tenantId, Edge edge, long ts, TbMsgType msgType) {
        try {
            ...
            if (isConnected) {
                edgeState.put(DefaultDeviceStateConstants.ACTIVITY_STATE, true);
                edgeState.put(DefaultDeviceStateConstants.LAST_CONNECT_TIME, ts);
            } else {
                edgeState.put(DefaultDeviceStateConstants.ACTIVITY_STATE, false);
                edgeState.put(DefaultDeviceStateConstants.LAST_DISCONNECT_TIME, ts);
            }
