...
public class EntityStateSourcingListener {
    ...
    /**
     * 处理实体保存事件 - 监听实体创建和更新操作
     *
     * @param event 保存实体事件，包含实体数据、租户信息、创建标志等
     * <p>
     * 处理流程：
     * 1. 检查是否需要广播事件（某些内部操作不需要广播）
     * 2. 验证实体ID和租户ID的有效性
     * 3. 根据实体类型分发到不同的处理方法
     * 4. 确定是创建还是更新操作，设置对应的生命周期事件
     * <p>
     * 事务特性：使用@TransactionalEventListener确保在事务提交后执行
     * fallbackExecution = true 表示即使没有事务上下文也执行
     */
    @TransactionalEventListener(fallbackExecution = true)
    public void handleEvent(SaveEntityEvent<?> event) {
        // 检查是否需要广播事件，某些内部操作可能设置不广播
