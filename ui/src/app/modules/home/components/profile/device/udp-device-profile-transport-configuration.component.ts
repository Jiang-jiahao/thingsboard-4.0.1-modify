...
export class UdpDeviceProfileTransportConfigurationComponent implements OnInit, AfterViewInit, OnDestroy, ControlValueAccessor, Validator {
  /** 手动 HEX 下「LTV/TLV 重复段」折叠面板：已配置 LTV 或用户勾选启用时默认展开 */
  hexLtvDefaultPathPanelExpanded = false;

  tcpHexValueTypes = UDP_HEX_FRAME_FIELD_VALUE_TYPES;
  /** LTV Tag→遥测映射下拉（静态 labelKey，供 translate 使用） */
  tcpHexLtvTagValueOptions = UDP_HEX_LTV_TAG_VALUE_OPTIONS;
  tcpHexMatchValueTypes = UDP_HEX_MATCH_VALUE_TYPES;
  tcpHexCommandMatchValueTypes = UDP_HEX_PROTOCOL_TEMPLATE_COMMAND_MATCH_TYPES;
  ...
  ngOnInit(): void {
    ...
    this.tcpDeviceProfileTransportConfigurationFormGroup = this.fb.group({
      ...
      dataType: [TransportUdpDataType.UTF8, Validators.required],
    ...
    this.tcpDeviceProfileTransportConfigurationFormGroup.get('udpWireAuthenticationMode').valueChanges.pipe(
      takeUntil(this.destroy$)
    ).subscribe((mode: UdpWireAuthenticationMode) => {
    ...
    this.tcpDeviceProfileTransportConfigurationFormGroup.get('dataType').valueChanges.pipe(
      takeUntil(this.destroy$)
    ).subscribe((dt: TransportUdpDataType) => {
      this.applyTcpOpaqueKeyValidators(dt);
      if (dt !== TransportUdpDataType.RAW_BYTES) {
    ...
    this.applyDeferredWireAuthTokenKeyValidators(
      this.tcpDeviceProfileTransportConfigurationFormGroup.get('udpWireAuthenticationMode').value
    );
  }

  private applyDeferredWireAuthTokenKeyValidators(mode: UdpWireAuthenticationMode): void {
    const keyCtrl = this.tcpDeviceProfileTransportConfigurationFormGroup.get('udpDeferredWireAuthTokenJsonKey');
    if (!keyCtrl) {
      return;
    }
  ...
  private applyTcpOpaqueKeyValidators(dt: TransportUdpDataType) {
    const keyCtrl = this.tcpDeviceProfileTransportConfigurationFormGroup.get('udpOpaqueRuleEngineKey');
    if (dt === TransportUdpDataType.UTF8 || dt === TransportUdpDataType.ASCII) {
      const normalized = String(keyCtrl.value ?? '').trim();
      keyCtrl.patchValue(normalized || 'udpOpaquePayload', { emitEvent: false });
  ...
  hexRequiresProtocolTemplateBundleHint(): boolean {
    ...
    if (!v || v.dataType !== TransportUdpDataType.RAW_BYTES) {
      return false;
    }
  ...
  writeValue(value: UdpDeviceProfileTransportConfiguration | null): void {
    if (isDefinedAndNotNull(value)) {
      ...
      ) || TransportUdpDataType.UTF8;
      const formDataType = rawType === TransportUdpDataType.PROTOCOL_TEMPLATE
      ...
      const hcfg = value.transportUdpDataTypeConfiguration;
      if (rawType === TransportUdpDataType.PROTOCOL_TEMPLATE && hcfg) {
        this.patchProtocolTemplatesFromModel(
      ...
      } else if (rawType === TransportUdpDataType.RAW_BYTES && hcfg) {
  ...
  private updateModel() {
    ...
    if (v.dataType === TransportUdpDataType.RAW_BYTES) {
      ...
      if (usePt) {
        ...
        if (templates.length) {
          transportUdpDataTypeConfiguration.protocolTemplates = templates;
        }
        ...
        if (commands.length) {
          transportUdpDataTypeConfiguration.protocolCommands = commands;
        }
        const bundleId = v.protocolTemplateBundleId;
        if (bundleId) {
        ...
        if (profiles.length) {
          transportUdpDataTypeConfiguration.hexCommandProfiles = profiles;
        }
        const defaultRows = (this.hexProtocolFieldsArray?.getRawValue() ?? []) as Array<Record<string, unknown>>;
        const defaultFields = this.fieldsFromHexRows(defaultRows);
        if (defaultFields.length) {
          transportUdpDataTypeConfiguration.hexProtocolFields = defaultFields;
        }
        if (v.hexLtvEnabled) {
          ...
          ltv.unknownTagTelemetryKeyHexLiteral = unknownTagTelemetryKeyHexLiteralFromMappings(tagMappings);
          transportUdpDataTypeConfiguration.hexLtvRepeating = ltv;
        }
        const manualCs = String(v.hexChecksumType ?? 'NONE').trim();
        if (manualCs && manualCs.toUpperCase() !== 'NONE') {
          transportUdpDataTypeConfiguration.checksum = {
            type: manualCs,
    ...
    const configuration: UdpDeviceProfileTransportConfiguration = {
      ...
      udpWireAuthenticationMode: v.udpWireAuthenticationMode,
      udpJsonWithoutMethodMode: UdpJsonWithoutMethodMode.TELEMETRY_FLAT,
      udpOpaqueRuleEngineKey: textLikePayload
        ? (String(v.udpOpaqueRuleEngineKey ?? '').trim() || 'udpOpaquePayload')
        : undefined,
  ...
  validate(): ValidationErrors | null {
    if (!this.tcpDeviceProfileTransportConfigurationFormGroup.valid) {
      return {udpProfileTransport: true};
    }
    const v = this.tcpDeviceProfileTransportConfigurationFormGroup.getRawValue();
    const dataType = v.dataType;
    const usePt = this.usesProtocolTemplatePayload(v as Record<string, unknown>);
    if (dataType === TransportUdpDataType.RAW_BYTES && !usePt && !this.hasManualHexConfiguration()) {
      return {tcpHexRequiresProtocolTemplateBundle: true};
    }
    if (dataType === TransportUdpDataType.RAW_BYTES && !usePt) {
