...
public class CalculatedFieldEntityMessageProcessor extends AbstractContextAwareMsgProcessor {
    ...
    // 每个计算字段处理完成后需要两个回调：一个用于持久化结果，一个用于持久化状
    ...
    CalculatedFieldEntityMessageProcessor(ActorSystemContext systemContext, TenantId tenantId, EntityId entityId) {
        ...
        this.cfService = (CalculatedFieldProcessingService) systemContext.getCalculatedFieldProcessingService();
        this.cfStateService = (CalculatedFieldStateService) systemContext.getCalculatedFieldStateService();
    }
    ...
    public void process(EntityCalculatedFieldLinkedTelemetryMsg msg) throws CalculatedFieldException {
        ...
        try {
            ...
            if (cfIds.contains(ctx.getCfId())) {
                ...
                // 根据消息类型（遥测、属性、删除）将数据转换为参数条目，并更新状
     ...
     * @param callback    回调
     * @param newArgValues 新的参数值映射（参数 -> 参数条目
    ...
    /**
     * 获取或初始化指定计算字段的状态
     * 先从本地内存获取，若不存在则从数据库异步加载（同步等待最1分钟）
     */
    @SneakyThrows
    private CalculatedFieldState getOrInitState(CalculatedFieldCtx ctx) {
        ...
        if (state != null) {
            ...
            // 阻塞等待状态加载完成，因为此操作发生频率很低（每个<实体，计算字>对只发生一次）
