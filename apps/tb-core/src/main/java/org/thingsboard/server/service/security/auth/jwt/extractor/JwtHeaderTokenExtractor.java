...
public class JwtHeaderTokenExtractor implements TokenExtractor {
    ...
    public String extract(HttpServletRequest request) {
        String header = request.getHeader(ThingsboardSecurityConstants.JWT_TOKEN_HEADER_PARAM);
        if (StringUtils.isBlank(header)) {
            header = request.getHeader(ThingsboardSecurityConstants.JWT_TOKEN_HEADER_PARAM_V2);
            if (StringUtils.isBlank(header)) {
