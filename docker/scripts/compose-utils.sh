...
function additionalComposeArgs() {
    ...
    case $DATABASE in
        postgres)
        ADDITIONAL_COMPOSE_ARGS="-f ../postgres/postgres.yml"
        ;;
        hybrid)
        ADDITIONAL_COMPOSE_ARGS="-f ../postgres/hybrid.yml"
        ;;
        *)
...
function additionalComposeMonitoringArgs() {
    ...
    then
      ADDITIONAL_COMPOSE_MONITORING_ARGS="-f ../monitoring/prometheus-grafana.yml"
...
function additionalComposeCacheArgs() {
    ...
    case $CACHE in
        redis)
        CACHE_COMPOSE_ARGS="-f ../redis/redis.yml"
        ;;
        redis-cluster)
        CACHE_COMPOSE_ARGS="-f ../redis/redis-cluster.yml"
        ;;
        redis-sentinel)
        CACHE_COMPOSE_ARGS="-f ../redis/redis-sentinel.yml"
        ;;
        *)
