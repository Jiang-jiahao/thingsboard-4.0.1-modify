 ...
 * SERVER 模式：
 * <ul>
 *   <li>优先在设备档案配置 {@code udpProfileServerBindPort}；未配置档案端口时可在此配置 {@link #serverBindPort}（兼容旧数据）。</li>
 ...
 *   {@link #udpWireAuthPayloadDeviceId}：与负载 JSON 中档案所配字段值一致，且在同一专用监听端口下多设备时值须互异。</li>
 * </ul>
 */
@Data
@ToString(of = {"host", "port", "sourceHost", "serverBindPort", "udpWireAuthPayloadDeviceId"})
public class UdpDeviceTransportConfiguration implements DeviceTransportConfiguration {
    ...
    /**
     * 兼容旧版：设备级专用监听端口。若设备档案已配置 {@code udpProfileServerBindPort}，此处须留空，由档案统一指定入口端口。
     */
    ...
    public void validate() {
        if (!isValid()) {
            throw new IllegalArgumentException("Udp transport: set host+port for CLIENT, or sourceHost / serverBindPort for SERVER");
        }
    }
    @JsonIgnore
