 ...
 */
package org.thingsboard.server.transport.tcp.limits;
...
public class IpFilter extends AbstractRemoteAddressFilter<InetSocketAddress> {
    ...
    public IpFilter(TcpTransportContext context) {
    ...
    protected boolean accept(ChannelHandlerContext ctx, InetSocketAddress remoteAddress) throws Exception {
        ...
        if(context.checkAddress(remoteAddress)){
            ...
            ctx.channel().attr(TcpTransportService.ADDRESS).set(remoteAddress);
            return true;
        } else {
            return false;
        }
