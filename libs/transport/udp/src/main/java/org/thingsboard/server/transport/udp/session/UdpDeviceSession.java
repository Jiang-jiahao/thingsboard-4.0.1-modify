...
public class UdpDeviceSession extends DeviceAwareSessionContext implements SessionMsgListener {
    ...
    public void sendAuthFrame(String token) {
        ByteBuf buf = UdpPayloadUtil.encodeAuthFrame(token);
        writeByteBuf(buf);
    }
    ...
    public UdpWireAuthenticationMode getUdpWireAuthenticationMode() {
        DeviceProfile profile = getDeviceProfile();
        if (profile == null || profile.getProfileData() == null || profile.getProfileData().getTransportConfiguration() == null) {
            return UdpWireAuthenticationMode.TOKEN;
        }
        var tc = profile.getProfileData().getTransportConfiguration();
        if (tc instanceof UdpDeviceProfileTransportConfiguration) {
    ...
    public boolean shouldSendWireAuthPayload() {
        return getUdpWireAuthenticationMode() == UdpWireAuthenticationMode.TOKEN;
    }
    ...
    public boolean isDeferredPayloadWireAuth() {
        UdpWireAuthenticationMode m = getUdpWireAuthenticationMode();
        return m == UdpWireAuthenticationMode.DEFERRED_PAYLOAD_TOKEN
                || m == UdpWireAuthenticationMode.DEFERRED_PAYLOAD_DEVICE_ID;
    }
